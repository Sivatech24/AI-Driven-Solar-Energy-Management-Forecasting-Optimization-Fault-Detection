...
# Preprocessing
scaler = MinMaxScaler()
df_scaled = scaler.fit_transform(gen_data[["DC_POWER"]])
...
if page in ["LSTM", "XGBoost", "Random Forest", "Linear Regression"]:
    st.title(f"{page} Model Training & Prediction")
    ...
    if page == "LSTM":
        ...
        model.fit(X_flat[:len(y_train)], y_train)
        predictions = model.predict(X_flat[len(y_train):])
    ...
    ax.plot(gen_data["DATE_TIME"][-len(y_test):], scaler.inverse_transform(y_test.reshape(-1, 1)), label="Actual", color="blue")
    ax.plot(gen_data["DATE_TIME"][-len(y_test):], predictions, label="Predicted", color="red")
    ax.set_title(f"{page} Predictions vs. Actual")
    ax.legend()
    st.pyplot(fig)